...
class HttpWebServer:
    def __init__(self, port):
        ...
        # 循环等待接受客户端的连接请求
        # 把tcp服务端的套接字当作web服务器的属性,这样在其他方法中可以使用,注意这样的调用方法没有参数传入在一般的init构造函数中
        self.tcp_server_socket = tcp_server_socket

    @staticmethod
    # 没有使用类对象也没有使用类函数类属性
    def handle_client_request(new_socket):  # 定义一个handle_client_request函数,用于处理客户端的请求
        # 代码执行到此说明连接建立成功
        # 接受客户端的请求信息
    ...
    def start(self):
        while True:
            ...
            # 当子线程创建完,设置子线程为守护线程,防止主线程由于阻塞无法关闭
